...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1" />
  <title>GO INSIDE • Form Terlambat Sekolah</title>

  <script src="https://cdn.tailwindcss.com"></script>
  <script src="https://unpkg.com/phosphor-icons"></script>
  <style>
    .btn-blue{ background-color:#60a5fa; color:#fff; transition:.2s }
    .btn-blue:hover{ background-color:#3b82f6 }
  </style>
</head>
...
<a href="{{ route('profilpiket.show', 1) }}" 
  ...
  <section class="flex-1">
    <div class="bg-white shadow-md rounded-3xl p-6">
      <h2 class="text-2xl font-extrabold mb-6 text-center">Form Terlambat Sekolah</h2>

      <form method="POST" action="{{ route('telat.store') }}" class="space-y-4">
        @csrf
        ...
        <div>
          ...
          <input type="text" name="nama_siswa" placeholder="Masukkan nama siswa"
            class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-400" required>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-semibold mb-1">Jam Masuk Sekolah</label>
            <input type="time" name="waktu_datang"
              class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-400" required>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name="alasan" rows="3" placeholder="Jelaskan alasan telat..."
            class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-400"></textarea>
        </div>

        <!-- Hukuman -->
        <div>
          <label class="block text-sm font-semibold mb-1">Hukuman</label>
          <input type="text" name="hukuman" placeholder="Misalnya: push-up 10x, menyapu kelas..."
            class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:ring-2 focus:ring-blue-400">
        </div>
        ...
        <div>
          ...
          <input type="text" id="noHpGuru" readonly
            class="w-full px-4 py-2 rounded-xl border border-slate-300 bg-slate-100"
            placeholder="Muncul otomatis setelah pilih guru">
        </div>

        <!-- Tombol -->
        <div class="text-center">
          <button type="submit" class="btn-blue px-6 py-2 rounded-xl font-bold">Ajukan</button>
        </div>
      </form>
    </div>
  </section>
